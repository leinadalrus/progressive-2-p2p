...
int hashed_callback_with_flush(int(function)(char *x, char *y, char *z,
  ...
  fcloseall(); // don't forget to flush!
...
int error_profound_command_handle(int(context)(char *, char *, char *,
  ...
  // void *reint_err_ptr = reinterpret_cast<void
  // *>(static_cast<intptr_t>(err_val));
...
const int *handle_user_input() {
  ...
  if (hashed_callback_with_flush == error_profound_command_handle)
    return retval;

  void *table_data, *data_input, *data_output;
  size_t data_size;

  dimmdir_note_throughput(table_data, data_input, data_output, data_size);

  UserData user_data = UserData{.source = table_data,
                                .destination = data_input,
                                .user_data = data_output,
                                .data_size = data_size};
}
